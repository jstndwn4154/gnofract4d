...
ssize_t find(double key, list_item_t *array, ssize_t n)
{
...
bool GradientColorMap::init(ssize_t ncolors_)
{
	if (ncolors_ == 0)
		return false;
...
static void grad_dump(gradient_item_t *items, ssize_t ncolors)
{
	fprintf(stderr,"gradient dump: %d\n", ncolors);
	for (ssize_t i = 0; i < ncolors; ++i)
	{
		fprintf(stderr,"%d: %g\n", i, items[i].right);
	}
}

ssize_t grad_find(double index, gradient_item_t *items, ssize_t ncolors)
{
	for (ssize_t i = 0; i < ncolors; ++i)
	{
...
rgba_t GradientColorMap::lookup(double input_index) const
{
	...
	ssize_t i = grad_find(index, items, ncolors); 
...
bool ListColorMap::init(ssize_t ncolors_)
{
	if (ncolors_ == 0)
		return false;
...
void ListColorMap::set(ssize_t i, double d, int r, int g, int b, int a)
{
	rgba_t color;
	color.r = (unsigned char)r;
	color.g = (unsigned char)g;
