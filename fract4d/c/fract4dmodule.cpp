...
public:
	...
	AutoBool(const int &i)
	{
		*this = i;
	}

	AutoBool(const bool &b)
	{
		value = b;
	}

	AutoBool(const AutoBool &a)
	{
		*this = a;
	}

	AutoBool & operator =(const int &b)
	{
		value = (b != 0);
		return *this;
	}

	AutoBool & operator =(const AutoBool &b)
	{
		if (this != &b)
			value = b.value;
		return *this;
	...
	// The following operators are not currently used, but could come in handy if
	// the elements powered by this class become more complex and need them

	/*bool operator ==(const AutoBool &b) const
	{
		return value == b.value;
	}
	bool operator ==(const int &b) const
	{
		return *this == AutoBool(b);
	}
	...
	AutoBool operator |(const AutoBool &b) const
	{
		return AutoBool(value | b.value);
	}
	AutoBool operator |(const int &b) const
	{
		return *this | AutoBool(b);
	}
	...
	AutoBool operator !() const
	{
		return AutoBool(!value);
	}
	AutoBool operator ~() const
	{
		return !*this;
...
public:
	...
	AutoBool auto_deepen, yflip, periodicity;
	AutoBool auto_tolerance, dirty;
	double tolerance;
	AutoBool async;
	int warp_param;
...
fw_find_root(PyObject *self, PyObject *args)
{
	...
	return Py_BuildValue("i(dddd)", ok, root[0], root[1], root[2], root[3]);
}
...
ff_create(PyObject *self, PyObject *args)
{
	...
	AutoBool auto_deepen, periodicity;
	AutoBool yflip;
	render_type_t render_type;
	pf_obj *pfo;
	ColorMap *cmap;
	...
	IFractalSite *site;
	IFractWorker *worker;
	AutoBool auto_tolerance;
	double tolerance;

	if (!PyArg_ParseTuple(args, "(ddddddddddd)iiiiOOiiiOOOid",
			&params[0], &params[1], &params[2], &params[3],
			&params[4], &params[5], &params[6], &params[7],
			&params[8], &params[9], &params[10],
			&eaa, &maxiter, &yflip, &nThreads,
			&pypfo, &pycmap, &auto_deepen,
			&periodicity, &render_type,
			&pyim, &pysite, &pyworker,
	...
	fractFunc *ff = new fractFunc(params, eaa,
			maxiter, nThreads, auto_deepen,
			auto_tolerance, tolerance, yflip,
			periodicity, render_type, -1, // warp_param
			worker, im, site);
	...
	PyObject *pyret = PyCObject_FromVoidPtr(ffh, (void (*)(void *))ff_delete);
